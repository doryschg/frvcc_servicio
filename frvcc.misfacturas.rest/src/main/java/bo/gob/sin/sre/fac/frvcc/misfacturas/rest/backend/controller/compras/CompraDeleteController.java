...
public class CompraDeleteController extends ApiController {
	@Autowired
	private IMensajeAplicacionDomain mensajesDomain;
    public CompraDeleteController(QueryBus queryBus, CommandBus commandBus) {
        super(queryBus, commandBus);
    }

    @DeleteMapping(value = "/compra/eliminar/{id}")
    public ResponseEntity<ResultadoGenericoDto<String>> index(@PathVariable String id) throws CommandNotRegisteredError, CommandHandlerExecutionError {
         dispatch(new EliminarCompraCommand(id,usuarioId()));
