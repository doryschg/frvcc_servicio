...
import bo.gob.sin.sre.fac.frvcc.misfacturas.adapters.primary.compras_notas.asociar_libro.AsociarLibroCompraNotaCommand;
...
public final class CompraNotaAsociarLibroPutController extends ApiController {
	@Autowired
	private IMensajeAplicacionDomain mensajesDomain;
	
    public CompraNotaAsociarLibroPutController(QueryBus queryBus, CommandBus commandBus) {
        super(queryBus, commandBus);
    }

    @PutMapping(value = "/compraNota/asociar/{id}")
    public ResponseEntity<ResultadoGenericoDto<String>> index(@PathVariable String id, @RequestBody Request request) throws CommandNotRegisteredError, CommandHandlerExecutionError {
         dispatch(new AsociarLibroCompraNotaCommand(id,
                 request.libroId(),
                 usuarioId()));
