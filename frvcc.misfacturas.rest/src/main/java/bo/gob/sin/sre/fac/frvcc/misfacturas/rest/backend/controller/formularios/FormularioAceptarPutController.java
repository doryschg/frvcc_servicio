...
import bo.gob.sin.sre.fac.frvcc.misfacturas.adapters.primary.formularios.consolidar.AceptarFormularioCommand;
...
public class FormularioAceptarPutController extends ApiController {
	@Autowired
	private IMensajeAplicacionDomain mensajesDomain;
  public FormularioAceptarPutController(QueryBus queryBus, CommandBus commandBus) {
        super(queryBus, commandBus);
    }

    @PutMapping(value = "/formulario/aceptar/")
    public ResponseEntity<ResultadoGenericoDto<String>> index(@RequestBody Request request) throws CommandNotRegisteredError, CommandHandlerExecutionError {
        dispatch(new AceptarFormularioCommand(request.id(),
                usuarioId(),
        ...
        ResultadoGenericoDto<String> resultadoGenericoDto = new ResultadoGenericoDto<>(request.id);
 		...
 		resultadoGenericoDto.getMensajes().add(mensajesDomain.getMensaje(EnumSubsistema.RECAUDACIONES,EnumFacFrvccMensajesShared.FORMULARIO_CONSOLIDADO_CORRECTO));
